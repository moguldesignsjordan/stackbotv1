...
const translations = {
  es: {
    ...
    errors: {
      ...
      emailInUse: 'Este correo ya está registrado. Inicia sesión en su lugar.',
      accountCreationFailed: 'Error al crear la cuenta. Intenta de nuevo.',
    },
...
export default function DriverApplyPage() {
  ...
  // ── auth listener (no redirect — new drivers aren't logged in yet) ──
  useEffect(() => {
  ...
  const validateForm = (): boolean => {
    ...
    // Password validation only when not already logged in
    if (!currentUser) {
    ...
    try {
      ...
      if (!currentUser) {
        try {
          ...
          setCurrentUser({ uid, email: userEmail });
        } catch (authErr: any) {
        ...
        <div className="relative flex-1 flex flex-col items-center justify-center px-4 py-8 safe-top safe-bottom">
          <div className="w-full max-w-md text-center">
            ...
            <div className="space-y-3">
              ...
              <button onClick={() => { setSubmitted(false); setFormData(initialFormData); setStep(1); setSignature(''); setAgreementAccepted(false); setHasScrolledToBottom(false); setPassword(''); setConfirmPassword(''); }} className="w-full py-3 text-white/70 hover:text-white transition-colors">
          ...
          <>
            ...
            <div className="bg-white rounded-2xl p-6 md:p-8 shadow-2xl">
              ...
              <form onSubmit={handleNextStep} className="space-y-6">
                ...
                {!currentUser ? (
                  <div className="bg-sb-primary/5 border border-sb-primary/20 rounded-xl p-4 space-y-4">
                    <div>
                      <h3 className="text-sm font-semibold text-gray-900">{t.form.accountSection}</h3>
                      <p className="text-xs text-gray-500">{t.form.accountSectionDesc}</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">{t.form.password} *</label>
                        <div className="relative">
                          ...
                          <input
                            ...
                            required
                            className="w-full pl-11 pr-11 py-3 bg-white border border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-sb-primary focus:border-transparent transition-all"
                          />
                          <button
                            type="button"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">{t.form.confirmPassword} *</label>
                        <div className="relative">
                          <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                          <input
                            type={showPassword ? 'text' : 'password'}
                            ...
                            required
                            className="w-full pl-11 pr-4 py-3 bg-white border border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-sb-primary focus:border-transparent transition-all"
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-xl">
                    ...
                    <p className="text-sm text-green-700">{t.form.loggedInAs} <span className="font-medium">{currentUser.email}</span></p>
                  </div>
                )}
